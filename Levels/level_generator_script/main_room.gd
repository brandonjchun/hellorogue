...
func _ready():
	...
	if PlayerData.levels >= 0 and PlayerData.levels < 3:
		PlayerData.sound_selecter = 0
	if PlayerData.levels >= 3 and PlayerData.levels < 6:
		PlayerData.sound_selecter = 1
	if PlayerData.levels >= 6 and PlayerData.levels < 9:
		PlayerData.sound_selecter = 2
	if PlayerData.levels >= 9 and PlayerData.levels < 12:
		PlayerData.sound_selecter = 3
	if PlayerData.levels >= 12 and PlayerData.levels <= 15:
		PlayerData.sound_selecter = 4
	if PlayerData.levels >= 15 and PlayerData.levels <= 18:
		PlayerData.sound_selecter = 5
...
func _process(delta):
	...
	match PlayerData.sound_selecter:
		0:
...
func generate_level(tilemap):
	walker = WalkerRoom.new(Vector2(3 + floor(lev/3), 5 + floor(lev/3)), borders)
...
func instance_silverspikes():
	var silverspikes_count = randi_range(PlayerData.levels, 2*PlayerData.levels)
...
func instance_redspikes():
	var redspikes_count = randi_range(PlayerData.levels, 2*PlayerData.levels)
